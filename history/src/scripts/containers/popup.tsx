...
import {Store, Page, Histories, History} from '../reducers';
...
const PopupContainer = (props: StateProps & DispatchProps) => {
  ...
  const {page, histories, openHistory, removeHistory, removeAllHistories} = props;
  ...
  } else if (isValidPageHistoryPair(page, histories) && histories.items.length) {
    content = (
  ...
  const onRemoveAll = () => page.result && removeAllHistories({host: page.result.host});

...
const BookmarkTreePage = ({className, bookmark, hover, onRemove}: Props) => {
  ...
  const onRemoveHandler = () => onRemove(bookmark);
  return (
    <li className={`${className} ${hover ? className + '_hover' : ''} bookmark-tree-page`}>
      <div className="bookmark-tree-page__title">{bookmark.title}</div>
      <div className="bookmark-tree-page__path" title={url}>{path}</div>
